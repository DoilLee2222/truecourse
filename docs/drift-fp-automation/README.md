...
runs the LLM stages via the **`--llm agent`** transport (the routine answers the prompts itself —
no `claude` binary or key; see root README "LLM transport"). See
...
>
> Generation works in the cloud **without** the `claude` binary or an API key via the **`--llm
> agent`** transport: truecourse's only LLM provider spawns the `claude` CLI (`ClaudeCodeProvider`,
> not documented to be present in the Routines env), so instead the tool surfaces each prompt +
> schema and the routine's own Claude answers it — behind the existing `BlockRunner`/`SliceRunner`
> injection seam. (Planned; until it ships, a maintainer can generate locally and push the same
> storage branch.)
...
| LLM | never | once per campaign, in `drift-fp-generate` (`--llm agent`); the discover/fix/close loop is LLM-free |
...
1. **drift-fp-generate** (the only LLM routine) picks the next `pending` campaign with no storage
   ...
   `spec scan → spec resolve --all-defaults → contracts generate` **via the `--llm agent`
   transport** (the routine answers the prompts itself — no `claude` subprocess, no key; see root
   README "LLM transport"), and **commits** the generated **specs + `.tc` contracts + scope +
...
│ spec scan + contracts generate via --llm agent│
...
**Maintainer fallback** (until `--llm agent` ships, or to seed a campaign by hand): run the stages
locally where `claude` is signed in and push the same storage branch + open the storage PR:
...
not a yaml flag, is the "already generated" signal), runs the LLM stages via `--llm agent`, commits
specs + contracts to a new `claude/drift-fp-store/…` branch, and opens the storage PR (label
`drift-fp-store`, **never merged**). Requires the `--llm agent` transport; if unavailable it posts
a blocker and stops (maintainer seeds the storage branch by hand).
...
5. **Bootstrap** by clicking **Run now** on `drift-fp-generate`. It generates the contracts and
   ...
   *(If `--llm agent` isn't implemented yet, seed the first campaign's storage branch locally —
   see [Generating & storing contracts](#generating--storing-contracts) — and open the storage PR
   by hand.)*
...
1. **Generate once, freeze on a storage branch.** The LLM stage (`spec scan` + `contracts
   generate`) runs once per campaign in `drift-fp-generate` (via `--llm agent`) and is committed to
...
1. Prompts are committed under `docs/drift-fp-automation/prompts/`: `drift-fp-generate.md`,
   `drift-fp-discover.md`, `drift-fp-next-fix.md`, `drift-fp-campaign-close.md`.
2. Implement the `--llm agent` transport (root README "LLM transport") so `drift-fp-generate` can
   run headless. Until then, seed the first campaign's storage branch locally (strapi/strapi —
   scope + `.tc` already produced by the OSS validation run).
3. Walk the setup checklist; create the five routines on the Default environment.
4. **Run now** on `drift-fp-generate` (or push a hand-seeded storage branch + open the storage PR)
   to start the first campaign.
